    ...
    private void MergeSeedData()
    {
        try
        {
            ...
            foreach (var seed in data.Profiles)
            {
                ...
                // Confirmed values in the seed are adopted by any profile that
                // has not been confirmed locally. Without this, a mob the user
                // has merely walked past once can never receive the shipped
                // confirmed numbers - and in the public build, which shows only
                // confirmed mobs, that mob would silently vanish from the list
                // for exactly the users who have played the most.
                //
                // A locally locked profile is never touched: the user's own
                // confirmation outranks anything baked in at build time.
                if (seed.Locked && !mine.Locked)
                {
                    mine.Locked          = true;
                    mine.LockedRange     = seed.LockedRange;
                    mine.LockedArcDegrees = seed.LockedArcDegrees;
                    mine.LockedIsSound   = seed.LockedIsSound;
                    filled = true;
                }
