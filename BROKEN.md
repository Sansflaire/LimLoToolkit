...
| 011 | Mob silhouettes never rendered once, and the only switch for them was buried in a tool panel rather than Settings | A ClientStructs *property* is not necessarily a field — `DrawObject.OutlineColor` is a bitfield in the high nibble of `OutlineFlags`, so writing it set half a byte and nothing drew. Read the accessor (shipped `.xml`, or the IL) before writing through one, and use the API ClientStructs' own docs recommend — here `GameObject.Highlight`. Separately: a feature whose switch is not where its neighbours live is a feature that does not exist | [011-outline-bitfield-half-written.md](Issues/011-outline-bitfield-half-written.md) |
